...
protected:
	...
	std::vector<std::string> ExecuteScript(void** obj, std::vector<std::string> step, std::vector<std::string> params)
	{
		std::vector<std::string> ret;
		size_t len1 = step.size();
		size_t len2 = params.size();
		if(len1 == len2){
			...
			std::vector<std::string>::iterator it = ret.begin();
			for(int i = 0; i < len1; i++){
				std::vector<std::string> passedParam(1, "");
				passedParam[0] = params.at(i);
				*(it + i) = ConductMethod(obj, step.at(i), passedParam);
			}
			std::cout << std::endl;
		}
		return ret;
